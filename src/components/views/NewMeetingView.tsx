...
import { extractMeetingData } from '../../services/extractor';
...
export const NewMeetingView: React.FC<NewMeetingViewProps> = ({ onMeetingCreated }) => {
  ...
  const handleAnalyze = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setAnalysisStage('2. Grounding commitments, decisions, and open issues...');
      const extraction = await extractMeetingData(transcript, title);
      ...
      // Save to persistent storage with cross-meeting carry-over linking
    ...
    <div className="p-6 md:p-8 max-w-4xl mx-auto space-y-8">
      ...
      <div className="space-y-2">
        ...
        <p className="text-sm text-slate-400">
          Upload or paste any transcript. The AntiGravity AI engine will parse decisions, action items, owners, deadlines, and unresolved blockers while preserving verbatim transcript evidence citations.
        </p>
      </div>
      ...
      <form onSubmit={handleAnalyze} className="space-y-6">
        ...
        {isAnalyzing ? (
          <div className="p-6 rounded-2xl bg-indigo-950/20 border border-indigo-500/30 text-center space-y-3 animate-pulse">
            ...
            <div className="text-sm font-semibold text-white">Analyzing Meeting with AI Engine...</div>
